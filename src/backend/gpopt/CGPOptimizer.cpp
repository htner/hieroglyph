...
//---------------------------------------------------------------------------
//	@function:
//		CGPOptimizer::GPOPTOptimizedPlanBoth
//
//	@doc:
//		Optimize given query using GP optimizer
//
...
)
{
	...
	*had_unexpected_failure = false;
	...
	{
		char* out_plan_str;
		plStmt = COptTasks::GPOPTOptimizedPlanBoth(query, &gpopt_context, &out_plan_str);
		*plan_str = pstrdup(out_plan_str);
		// clean up context
		gpopt_context.Free(gpopt_context.epinQuery, gpopt_context.epinPlStmt);
	}
	GPOS_CATCH_EX(ex)
...
PlannedStmt *
GPOPTOptimizedPlanBoth(Query *query, char** plan_str, bool * had_unexpected_failure)
{
	return CGPOptimizer::GPOPTOptimizedPlanBoth(query, plan_str, had_unexpected_failure);
}
