...
static Aws::S3::S3Client *create_s3_connection(ForeignServer *server,
                                               UserMapping *user,
                                               bool use_minio);
 ...
 */
Aws::S3::S3Client *parquetGetConnection(UserMapping *user, bool use_minio) {
  bool found;
  ...
  if (ConnectionHash == NULL) {
    ...
    ConnectionHash = hash_create("parquet_s3_fdw connections", 8, &ctl,
     ...
     */
    CacheRegisterSyscacheCallback(FOREIGNSERVEROID, parquet_fdw_inval_callback,
                                  (Datum)0);
    CacheRegisterSyscacheCallback(USERMAPPINGOID, parquet_fdw_inval_callback,
                                  (Datum)0);
  }
   ...
   */
  entry =
      (ConnCacheEntry *)hash_search(ConnectionHash, &key, HASH_ENTER, &found);
  if (!found) {
    /*
  ...
  if (entry->conn != NULL && entry->invalidated) {
    elog(DEBUG3,
         "parquet_s3_fdw: closing handle %p for option changes to take effect",
         entry->conn);
   ...
   */
  if (entry->conn == NULL) make_new_connection(entry, user, use_minio);
...
static void make_new_connection(ConnCacheEntry *entry, UserMapping *user,
  ...
  elog(DEBUG3,
       "parquet_s3_fdw: new parquet_fdw handle %p for server \"%s\" (user "
...
static bool disconnect_cached_connections(Oid serverid) {
   ...
   */
  if (!ConnectionHash) return false;
 ...
 */
static int ExtractConnectionOptions(List *defelems, const char **keywords,
                                    const char **values) {
  ListCell *lc;
  int i;
...
static Aws::S3::S3Client *create_s3_connection(ForeignServer *server,
  ...
  PG_TRY();
  {
    ...
    n = ExtractConnectionOptions(lst_options, keywords, values);
    ...
    foreach (lc, lst_options) {
      ...
      if (strcmp(def->defname, "user") == 0) id = defGetString(def);

      if (strcmp(def->defname, "password") == 0) password = defGetString(def);

      if (strcmp(def->defname, "region") == 0) awsRegion = defGetString(def);

      if (strcmp(def->defname, "endpoint") == 0) endpoint = defGetString(def);
    }
...
static void check_conn_params(const char **keywords, const char **values,
  ...
  for (i = 0; keywords[i] != NULL; i++) {
    if (strcmp(keywords[i], "password") == 0 && values[i][0] != '\0') return;
  }

  ereport(ERROR, (errcode(ERRCODE_S_R_E_PROHIBITED_SQL_STATEMENT_ATTEMPTED),
                  errmsg("parquet_s3_fdw: password is required"),
                  errdetail("parquet_s3_fdw: Non-superusers must provide a "
                            "password in the user mapping.")));
}
 ...
 */
Aws::S3::S3Client *parquetGetConnectionByTableid(Oid foreigntableid) {
 ...
 */
List *parquetGetS3ObjectList(Aws::S3::S3Client *s3_cli, const char *s3path) {
  List *objectlist = NIL;
  Aws::S3::S3Client s3_client = *s3_cli;
  Aws::S3::Model::ListObjectsRequest request;

  if (s3path == NULL) return NIL;
 ...
 */
void parquet_disconnect_s3_server() {
  HASH_SEQ_STATUS scan;
  ConnCacheEntry *entry;
  ...
  while ((entry_reader = (ReaderCacheEntry *)hash_seq_search(&scan_reader))) {
    /* Ignore cache entry if no open connection right now */
    if (entry_reader->file_reader == NULL) continue;

    elog(DEBUG3, "parquet_s3_fdw: discarding reader connection %p",
         entry_reader->file_reader);
 ...
 */
FileLocation parquetFilenamesValidator(const char *filename, FileLocation loc) {
  if (IS_S3_PATH(filename)) {
    if (loc == LOC_LOCAL)
      elog(ERROR,
           "parquet_s3_fdw: Cannot specify the mix of local file and S3 file");
    return LOC_S3;
  } else {
    if (loc == LOC_S3)
      elog(ERROR,
 ...
 */
bool parquetIsS3Filenames(List *filenames) {
  char *name;

  if (filenames == NIL) return false;
 ...
 */
List *parquetGetDirFileList(List *filelist, const char *path) {
  int ret;
  struct stat st;
  DIR *dp;
  struct dirent *entry;
  ...
  ret = stat(path, &st);
  if (ret != 0) elog(ERROR, "parquet_s3_fdw: cannot stat %s", path);
  ...
  if ((st.st_mode & S_IFMT) != S_IFDIR) return filelist;

  dp = opendir(path);
  if (!dp) elog(ERROR, "parquet_s3_fdw: cannot open %s", path);
...
List *parquetImportForeignSchemaS3(ImportForeignSchemaStmt *stmt,
  ...
  foreach (cell, objects) {
    ...
    if (ext && strcmp(ext + 1, "parquet") != 0) continue;
    ...
    *ext = '\0';
    ...
    query = create_foreign_table_query(filename, stmt->local_schema,
                                       stmt->server_name, &fullpath, 1, fields,
                                       stmt->options);
...
/*
 * Wrapper of extract_parquet_fields(). If the path is S3 URL, we give s3 handle
 * to extract_parquet_fields().
 */
List *parquetExtractParquetFields(List *fields, char **paths,
                                  const char *servername) noexcept {
  if (!fields) {
    if (IS_S3_PATH(paths[0])) {
      ForeignServer *fserver = GetForeignServerByName(servername, false);
      UserMapping *user = GetUserMapping(GetUserId(), fserver->serverid);
      parquet_s3_server_opt *options =
          parquet_s3_get_server_options(fserver->serverid);
      Aws::S3::S3Client *s3client =
          parquetGetConnection(user, options->use_minio);
 ...
 * if we don't already have a suitable one.
 */
ReaderCacheEntry *parquetGetFileReader(Aws::S3::S3Client *s3client, char *dname,
                                       char *fname) {
  bool found;
   ...
   */
  entry =
      (ReaderCacheEntry *)hash_search(FileReaderHash, &key, HASH_ENTER, &found);
  if (!found) {
    /*
  ...
  if (entry->file_reader == NULL || entry->file_reader->reader == nullptr) {
    ...
    elog(DEBUG3,
         "parquet_s3_fdw: new parquet file reader for s3handle %p %s/%s",
 ...
 */
extern "C" {
Datum parquet_s3_fdw_get_connections(PG_FUNCTION_ARGS) {
#define PARQUET_S3_FDW_GET_CONNECTIONS_COLS 2
  ...
  if (rsinfo == NULL || !IsA(rsinfo, ReturnSetInfo))
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg("parquet_s3_fdw: set-valued function called in "
                           "context that cannot accept a set")));
  if (!(rsinfo->allowedModes & SFRM_Materialize))
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg("parquet_s3_fdw: materialize mode required, but it "
 ...
 */
Datum parquet_s3_fdw_disconnect(PG_FUNCTION_ARGS) {
  ForeignServer *server;
  char *servername;
 ...
 */
Datum parquet_s3_fdw_disconnect_all(PG_FUNCTION_ARGS) {
