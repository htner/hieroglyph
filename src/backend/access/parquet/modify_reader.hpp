...
class ModifyParquetReader : ParquetReader {
 ...
 private:
  ...
  /* cache data of target parquet file, it will be whole schema if parquet file
   * does not exist */
 ...
 private:
  ...
  void *read_primitive_type_raw(arrow::Array *array, TypeInfo *type_info,
                                int64_t i);
  /* read an arrow column form table in `void **` type (column of `void *` data)
   */
  void read_column(std::shared_ptr<arrow::Table> table, int col,
                   TypeInfo *type_info, void ***data, bool **is_null);
  ...
  std::shared_ptr<arrow::ArrayBuilder> typeid_get_builder(
      const TypeInfo &typeinfo);
  /* append primitive value to array builder */
  std::shared_ptr<arrow::Array> builder_append_primitive_type(
      arrow::ArrayBuilder *builder, TypeInfo &type_info, void **column_values,
      bool *isnulls, size_t len, bool need_finished);
  /* create arrow table from cache data */
  std::shared_ptr<arrow::Table> create_arrow_table();
  /* write arrow table to a parqet file */
  void parquet_write_file(const char *dirname, Aws::S3::S3Client *s3_client,
                          const arrow::Table &table);

  /* CAST HELPER FUNCTIONS */
  /* init cast from postgres type to mapped parquet file */
  void initialize_postgres_to_parquet_cast(TypeInfo &typinfo,
                                           const char *attname);
  void exec_cast(std::vector<int> column_name, std::vector<Datum> &row_value,
                 std::vector<bool> is_nulls);

  /* INSERT HELPER FUNCTIONS */
  /* insert value to a column */
  void add_value_to_column(void ***column, bool **isnulls, size_t column_len,
                           TypeInfo &column_type, Datum value, bool value_null,
                           size_t idx);
  /* Get value from Datum and convert to void pointer */
  void postgres_val_to_voidp(const TypeInfo &type_info, Datum value,
                             void **res);
  ...
  /* true if sorted columns are bigger (or eq) than which in `row_idx - 1` and
   * smaller than which in (or eq) `row_idx` */
  bool is_right_position_in_sorted_column_datum(std::vector<int> attrs,
                                                std::vector<Datum> row_values,
                                                std::vector<bool> is_nulls,
                                                size_t row_idx);
  bool is_right_position_in_sorted_column_voidp(void **row, bool *is_nulls,
                                                size_t row_len, size_t row_idx);

  /* UPDATE/DELETE HELPER FUNCTION */
  /* true if row in idx match all key */
  bool is_modify_row(std::vector<int> key_attrs, std::vector<Datum> key_values,
                     size_t row_idx);
 ...
 public:
  ...
  ReadStatus next(TupleTableSlot *slot, bool fake = false) override {
    elog(
        ERROR,
        "parquet_s3_fdw: ModifyParquetReader does not support next() function");
  }
  void rescan() override {
    elog(ERROR,
         "parquet_s3_fdw: ModifyParquetReader does not support rescan() "
         "function");
  }
};

std::shared_ptr<ModifyParquetReader> create_modify_parquet_reader(
    const char *filename, MemoryContext cxt,
    std::shared_ptr<arrow::Schema> schema = nullptr, bool is_new_file = false,
    int reader_id = -1);
