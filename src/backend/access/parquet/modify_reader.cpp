 ...
 */
void ModifyParquetReader::initialize_postgres_to_parquet_cast(TypeInfo &typinfo, const char *attname)
{
 ...
 */
void ModifyParquetReader::open(const char *dirname, Aws::S3::S3Client *s3_client)
{
    arrow::Status status;
    char *dname;
    char *fname;
...
void ModifyParquetReader::close()
{
 ...
 */
void ModifyParquetReader::set_sorted_col_list(std::set<std::string> sorted_cols)
{
 ...
 */
void ModifyParquetReader::set_options(bool use_threads, bool use_mmap)
{
 ...
 */
void ModifyParquetReader::set_schemaless(bool schemaless)
{
 ...
 */
void ModifyParquetReader::set_keycol_names(std::set<std::string> keycol_names)
{
 ...
 */
bool ModifyParquetReader::compare_filename(char *filename)
{
    if (filename == NULL)
        return false;
    else
 ...
 */
bool ModifyParquetReader::schema_check(std::vector<int> attrs, std::vector<bool> is_nulls)
{
 ...
 */
void ModifyParquetReader::read_column(std::shared_ptr<arrow::Table> table,
 ...
 */
void ModifyParquetReader::create_new_file_temp_cache()
{
 ...
 */
void ModifyParquetReader::cache_parquet_file_data()
{
    std::shared_ptr<arrow::Table> tmptable;
 ...
 */
void ModifyParquetReader::postgres_val_to_voidp(const TypeInfo &type_info, Datum value, void **res)
{
    if (res == NULL)
        return;
 ...
 */
void ModifyParquetReader::add_value_to_column(void ***column, bool **isnulls, size_t column_len, TypeInfo &column_type, Datum value, bool value_null, size_t idx)
{
 ...
 */
template <typename builderTypeP, typename ValueType>
 ...
 */
void ModifyParquetReader::parquet_write_file(const char *dirname,
 ...
 * @param s3_client aws s3 client
 */
void ModifyParquetReader::upload(const char *dirname, Aws::S3::S3Client *s3_client)
{
 ...
 */
bool ModifyParquetReader::is_right_position_in_sorted_column_datum(std::vector<int> attrs, std::vector<Datum> row_values, std::vector<bool> is_nulls, size_t row_idx)
{
 ...
 * @return true if sorted columns are bigger (or eq) than which in `row_idx - 1` and smaller than which in (or eq) `row_idx`
 */
bool ModifyParquetReader::is_right_position_in_sorted_column_voidp(void **row, bool *is_nulls, size_t row_len, size_t row_idx)
{
    if (this->sorted_cols.size() == 0)
        return true;
 ...
 */
void ModifyParquetReader::exec_cast(std::vector<int> attrs, std::vector<Datum> &row_values, std::vector<bool> is_nulls)
{
    for (size_t attr_idx = 0; attr_idx < attrs.size(); attr_idx++)
    {
 ...
 */
bool ModifyParquetReader::schemaless_parse_column(Datum attr_value, std::vector<int> *attrs,
 ...
 */
bool ModifyParquetReader::is_modify_row(std::vector<int> key_attrs, std::vector<Datum> key_values, size_t row_idx)
{
 ...
 */
bool ModifyParquetReader::exec_delete(std::vector<int> key_attrs, std::vector<Datum> key_values)
{
 ...
 * @param row_idx row index
 */
void ModifyParquetReader::reorder_row(size_t row_idx)
{
    /* get row */
    void **row;
    bool *row_isnulls;
 ...
 */
bool ModifyParquetReader::exec_update(std::vector<int> key_attrs, std::vector<Datum> key_values,
 ...
 * @param schema target file schema
 */
void ModifyParquetReader::get_columns_type(std::shared_ptr<arrow::Schema> schema)
{
 ...
 */
void ModifyParquetReader::create_column_mapping(TupleDesc tupleDesc, const std::set<int> &attrs_used)
{
    std::shared_ptr<arrow::Schema> schema;
 ...
 */
void ModifyParquetReader::get_element_type_info(Oid type, const char *colname, TypeInfo &elem)
{
    int16 elem_len;
    bool elem_byval;
    char elem_align;
