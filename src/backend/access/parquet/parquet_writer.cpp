 ...
 */
std::shared_ptr<ParquetWriter> CreateParquetWriter(const char *filename,
...
void ParquetWriter::ParquetWriteFile(const char *dirname,
	...
	try {
		...
		if (s3_client) {
			bool uploaded = parquet_upload_file_to_s3(dirname, s3_client, filename_.c_str(), local_path.c_str());
