...
//auto GetValue(arrow::Array* a, int64_t i) -> decltype(arrow::TypeTraits<TYPE_CLASS>::CType) {
//decltype(auto) GetValue(arrow::Array* a, int64_t i) { //}-> decltype(CType) {
...
template<class TYPE_CLASS>
Datum GetStringToDatum(arrow::Array* a, int64_t i) {                     using ArrayType = typename arrow::TypeTraits<TYPE_CLASS>::ArrayType;
    auto array = reinterpret_cast<ArrayType*>(a);
...
template<>
arrow::Status GetDatum<FLOAT4OID>(arrow::Array* array, int64_t i, Datum* datum) {
...
template<>
arrow::Status GetDatum<FLOAT8OID>(arrow::Array* array, int64_t i, Datum* datum) {
...
template<>
arrow::Status GetDatum<CHAROID>(arrow::Array* array, int64_t i, Datum* datum) {
	*datum = GetStringToDatum<arrow::FixedSizeBinaryType>(array, i);
    return arrow::Status::OK();
}

template<>
arrow::Status GetDatum<BPCHAROID>(arrow::Array* array, int64_t i, Datum* datum) {
	*datum = GetStringToDatum<arrow::BinaryType>(array, i);
    return arrow::Status::OK();
}

template<>
arrow::Status GetDatum<VARCHAROID>(arrow::Array* array, int64_t i, Datum* datum) {
	*datum = GetStringToDatum<arrow::BinaryType>(array, i);
    return arrow::Status::OK();
}
...
template<>
arrow::Status GetDatum<TIMESTAMPOID>(arrow::Array* array, int64_t i, Datum* datum) {
	auto value = GetValue<arrow::Time64Type>(array, i);
	*datum = TimeADTGetDatum(value);
    return arrow::Status::OK();
}
...
template<>
arrow::Status GetDatum<TIMESTAMPTZOID>(arrow::Array* array, int64_t i, Datum* datum) {
	auto value = GetValue<arrow::Time64Type>(array, i);
	*datum = TimeADTGetDatum(value);
    return arrow::Status::OK();
}
...
template<>
arrow::Status GetDatum<TIMETZOID>(arrow::Array* array, int64_t i, Datum* datum) {
	auto value = GetValue<arrow::Time32Type>(array, i);
	*datum = DateADTGetDatum(value);
    return arrow::Status::OK();
}
...
arrow::Status GetArray(GetDatumFunc sub_func, 
					   Oid sub_type, 
					   int32_t elmlen,
					   bool elmbyval, 
					   char elmalign,
	...
	ArrayType* t = construct_array(datums,
					size, sub_type, elmlen, elmbyval, elmalign);
...
ColumnExchanger::ColumnExchanger(int16_t typid) {
	func_ = GetFunction(typid);
}
...
GetDatumFunc ColumnExchanger::GetFunction(Oid typid,
	...
	if (typtype == TYPTYPE_COMPOSITE &&typrelid != 0) {
		...
		return std::bind(GetStruct, sub_funcs, sub_types, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
	}
...
arrow::Status ColumnExchanger::GetDatumByIndex(int64_t i, Datum* datum, bool* isnull) {
	if (array_ == nullptr) {
