...
std::shared_ptr<arrow::DataType> TypeMapping::GetDataType(Form_pg_attribute attr) {
	...
	return GetDataType(atttypid, elem_type->typelem, elem_type->typrelid, elem_type->typlen, typmod, typtype);
}
...
std::shared_ptr<arrow::DataType> TypeMapping::GetDataType(Oid typid) {
	...
	for (;;) {
		...
		return GetDataType(typid, elem_type->typelem, elem_type->typrelid, elem_type->typlen, typmod, typtype); 
	}
	return nullptr;
...
std::shared_ptr<arrow::DataType> TypeMapping::GetDataType(Oid typid, 
	...
	if (typrelid != 0) {
		...
		for (int i = 0; i < tupdesc->natts; i++) {
			...
			sub_fields.push_back(std::make_shared<arrow::Field>(name, sub_data_type, !attr->attnotnull));
		}
