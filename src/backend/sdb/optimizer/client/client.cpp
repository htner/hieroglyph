...
int main(int argc, char* argv[]) {
    ...
    sdb::Optimizer_Stub stub(&channel);

    // Send a request and wait for the response every 1 second.
    while (!brpc::IsAskedToQuit()) {
        // We will receive response synchronously, safe to put variables
        // on stack.
        sdb::OptimizeRequest request;
        sdb::OptimizeReply response;
        brpc::Controller cntl;

        request.set_name("grpc_req_from_brpc");
        request.set_sql("select * from table1");
		request.set_sid(1);
		request.set_uid(1);
		request.set_dbid(1);
		request.set_xid(1);
        ...
        stub.Depart(&cntl, &request, &response, NULL);
        if (!cntl.Failed()) {
